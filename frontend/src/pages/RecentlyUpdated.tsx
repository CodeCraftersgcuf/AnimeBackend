...
const RecentlyUpdated = () => {
  ...
  const fetchData = async (page = 1) => {
    setLoading(true);
    try {
      const res = await fetch(`http://localhost:3030/api/v1/animes/recently-updated?page=${page}`);
  ...
  return (
    <div>
      <h1 className="text-blue-300 text-2xl font-bold mb-6">Recently Updated Anime</h1>
      {loading ? (
