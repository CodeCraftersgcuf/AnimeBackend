...
const TopUpcoming = () => {
  ...
  const fetchData = async (page = 1) => {
    setLoading(true);
    try {
      const res = await fetch(`http://localhost:3030/api/v1/animes/top-upcoming?page=${page}`);
  ...
  return (
    <div>
      <h1 className="text-purple-300 text-2xl font-bold mb-6">Top Upcoming Anime</h1>
      {loading ? (
