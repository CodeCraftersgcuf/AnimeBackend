...
const MostPopular = () => {
  ...
  const fetchData = async (page = 1) => {
    setLoading(true);
    try {
      const res = await fetch(`http://localhost:3030/api/v1/animes/most-popular?page=${page}`);
  ...
  return (
    <div>
      <h1 className="text-yellow-300 text-2xl font-bold mb-6">Most Popular Anime</h1>
      {loading ? (
