...
const MostFavorite = () => {
  ...
  const fetchData = async (page = 1) => {
    setLoading(true);
    try {
      const res = await fetch(`http://localhost:3030/api/v1/animes/most-favorite?page=${page}`);
  ...
  return (
    <div>
      <h1 className="text-pink-400 text-2xl font-bold mb-6">Most Favorite Anime</h1>
      {loading ? (
