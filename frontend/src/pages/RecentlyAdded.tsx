...
const RecentlyAdded = () => {
  ...
  const fetchData = async (page = 1) => {
    setLoading(true);
    try {
      const res = await fetch(`http://localhost:3030/api/v1/animes/recently-added?page=${page}`);
  ...
  return (
    <div>
      <h1 className="text-orange-300 text-2xl font-bold mb-6">Recently Added Anime</h1>
      {loading ? (
